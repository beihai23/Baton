...
一行命令，浏览器打开即用（WebUI 模式，常驻进程只有一个）：
...
首次运行会自动装前端依赖、构建 WebUI、编译 core（含 MCP 二进制）；之后启动即秒开。
数据库首次启动自动建库并播种演示数据。
...
- **顶栏**：审批（需要你裁决的队列）、通知（@提及/依赖解除/接管/移交等动态）、Agent 在场状态一目了然
...
| Agent | MCP（Claude Code 等） | 认领、执行、汇报、评论、移交、协同、验收；关注订阅 + 心跳信号让它**主动感知**变化 |
| 脚本/其他工具 | HTTP API / `baton` CLI | 一切。三者共享同一 SQLite 库，语义一致 |

**让 Agent 无人值守地主动介入**：`baton watch --cmd 'claude -p'` 守护进程会监听与你
Agent 相关的看板动态（@提及/移交/关注卡更新），自动唤起一个 Agent 进程去处理。
